 ...
 */
secp256k1_ecmult_big_context_t* secp256k1_ecmult_big_create(const unsigned int bits) {
    unsigned int windows;
    size_t window_size, total_size;
    size_t i, row;

    secp256k1_fe_t  fe_zinv;
    secp256k1_ge_t  ge_temp;
    secp256k1_ge_t  ge_window_one = secp256k1_ge_const_g;
    secp256k1_gej_t gej_window_base;
    secp256k1_ecmult_big_context_t *rtn;
    ...
    rtn = (secp256k1_ecmult_big_context_t *)checked_malloc(sizeof(secp256k1_ecmult_big_context_t));
    *(unsigned int *)(&rtn->bits) = bits;
    *(unsigned int *)(&rtn->windows) = windows;

    /* An array of secp256k1_ge_storage pointers, one for each window. */
    rtn->precomp = (secp256k1_ge_storage_t **)checked_malloc(sizeof(secp256k1_ge_storage_t *) * windows);
    ...
    rtn->precomp[0] = (secp256k1_ge_storage_t *)checked_malloc(sizeof(secp256k1_ge_storage_t) * total_size);

    /* Each row starts window_size elements after the previous. */
    for ( i = 1; i < windows; i++ ) { rtn->precomp[i] = (rtn->precomp[i - 1] + window_size); }

    rtn->gej_temp = (secp256k1_gej_t *)checked_malloc(sizeof(secp256k1_gej_t) * window_size);
    rtn->z_ratio  = (secp256k1_fe_t  *)checked_malloc(sizeof(secp256k1_fe_t ) * window_size);
 ...
 */
void secp256k1_ecmult_big_destroy(secp256k1_ecmult_big_context_t* bmul) {
 ...
 */
uint64_t secp256k1_scalar_shr_any(secp256k1_scalar_t *s, unsigned int n) {
 ...
 */
static int64_t secp256k1_scalar_sdigit_single(secp256k1_scalar_t *s, unsigned int w) {
    int64_t sdigit = 0;
 ...
 */
static size_t secp256k1_scalar_sdigit(int64_t *sdigits, secp256k1_scalar_t s, unsigned int w) {
    size_t digits = 0;
 ...
 */
static void secp256k1_ecmult_big(const secp256k1_ecmult_big_context_t* bmul, secp256k1_gej_t *r, const secp256k1_scalar_t *a) {
    size_t  window = 0;
    int64_t sdigit = 0;
    secp256k1_ge_t window_value;

    /* Copy of the input scalar which secp256k1_scalar_sdigit_single will destroy. */
    secp256k1_scalar_t privkey = *a;
