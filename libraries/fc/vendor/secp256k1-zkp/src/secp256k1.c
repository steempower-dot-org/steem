...
struct secp256k1_context_struct {
    ...
    secp256k1_ecmult_big_context_t *ecmult_big_ctx;
};
...
secp256k1_context_t* secp256k1_context_create(int flags) {
    ...
    ret->ecmult_big_ctx = secp256k1_ecmult_big_create(18);
...
void secp256k1_context_destroy(secp256k1_context_t* ctx) {
    ...
    secp256k1_ecmult_big_destroy(ctx->ecmult_big_ctx);
    ...
    if (!overflow && !secp256k1_scalar_is_zero(&sec)) {
        ...
        while (1) {
            ...
            if (!secp256k1_scalar_is_zero(&non) && !overflow) {
                if (secp256k1_ecdsa_sig_sign(ctx->ecmult_big_ctx, &sig, &sec, &msg, &non, NULL)) {
                    break;
                }
            }
            count++;
...
int secp256k1_ecdsa_sign_compact(const secp256k1_context_t* ctx, const unsigned char *msg32, unsigned char *sig64, const unsigned char *seckey, secp256k1_nonce_function_t noncefp, const void* noncedata, int *recid) {
    ...
    if (!overflow && !secp256k1_scalar_is_zero(&sec)) {
        ...
        while (1) {
            ...
            /* memset(nonce32, 0, 32); */
            if (!secp256k1_scalar_is_zero(&non) && !overflow) {
                if (secp256k1_ecdsa_sig_sign(ctx->ecmult_big_ctx, &sig, &sec, &msg, &non, recid)) {
                    break;
                }
            }
            count++;
